...
export function Hero() {
  ...
  return (
    <section
      id="home"
      ref={sectionRef}
      className="relative flex min-h-[92svh] items-center overflow-hidden bg-grid"
      aria-label={
        locale === "pt" ? "Introdução à Silentra" : "Silentra introduction"
      }
    >
      <motion.div
        aria-hidden="true"
      ...
      <div className="mx-auto w-full max-w-7xl px-4 pb-14 pt-24 sm:px-6 sm:pb-20 lg:px-8 lg:pt-28">
        <motion.div
          style={{ opacity }}
          className="grid items-center gap-12 lg:grid-cols-[minmax(0,1.02fr)_minmax(360px,0.98fr)] lg:gap-10"
        >
          <motion.div
            style={{ y: contentY }}
          ...
          >
            ...
            >
              <a
                ...
                className="group inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-semibold text-black transition hover:bg-white/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/60 sm:px-6"
              >
                {t.contact}
                <ArrowUpRight
                  size={16}
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.02] px-5 py-3 text-sm font-medium text-white transition hover:border-white/20 hover:bg-white/[0.05] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 sm:px-6"
              >
            ...
            <motion.div
              variants={itemVariants}
              className="grid max-w-xl grid-cols-3 gap-3 border-t border-white/[0.07] pt-5 sm:gap-7"
            >
              {t.stats.map(([value, label]) => (
                <div key={label} className="flex flex-col gap-1">
                  ...
                  <span className="text-[9px] uppercase tracking-[0.12em] text-[#656565] sm:text-[10px]">
                    {label}
                  </span>
                </div>
              ))}
          ...
          >
            <div className="relative">
              <div
                aria-hidden="true"
                className="absolute inset-0 scale-125 rounded-full bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.06)_0%,transparent_70%)] blur-2xl"
              />
      ...
      <motion.a
        ...
        style={{ opacity }}
        className="group absolute bottom-7 left-1/2 hidden -translate-x-1/2 items-center gap-2 text-[9px] uppercase tracking-[0.2em] text-[#666] transition hover:text-white sm:flex"
        aria-label={locale === "pt" ? "Ver serviços" : "View services"}
      >
        <span>{locale === "pt" ? "Explorar" : "Explore"}</span>
        <ArrowDown
          size={13}
