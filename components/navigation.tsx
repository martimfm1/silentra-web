...
import {
  RadialNav,
  type RadialNavItem,
} from "@/components/animate-ui/components/community/radial-nav";
...
export function Navigation() {
  ...
  const headerScale = useSpring(useTransform(scrollY, [0, 120], [1, 0.985]), {
    stiffness: 170,
    damping: 26,
  });
  ...
  const mobileRadialSize =
    viewport.width > 0
      ? Math.max(220, Math.min(280, viewport.width - 56, viewport.height - 190))
      : 280;
            ...
            >
              {[
                ...
                >
                  ...
                  <span
                    aria-hidden="true"
                    className="absolute inset-x-3 bottom-1 h-px origin-left scale-x-0 bg-white/50 transition-transform duration-300 group-hover:scale-x-100"
                  />
                </a>
              ))}
            </nav>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 flex items-center justify-center overflow-hidden bg-[#050505] px-4 pt-20 pb-10 md:hidden"
          >
            <motion.div
              aria-hidden="true"
              ...
              className="pointer-events-none absolute h-[min(74vw,340px)] w-[min(74vw,340px)] rounded-full bg-[radial-gradient(circle,rgba(255,255,255,0.06),transparent_65%)] blur-3xl"
            />
            <motion.div
            ...
            >
              <RadialNav
                ...
                items={radialItems}
                menuButtonConfig={{
                  iconSize: Math.max(12, Math.min(18, mobileRadialSize / 22)),
                  buttonSize: Math.max(
                    32,
                    Math.min(38, mobileRadialSize / 9.5),
                  ),
                  buttonPadding: 8,
                }}
                onActiveChange={(id) =>
                  handleNavClick(radialHrefs[id] ?? "#home")
                }
              />
            </motion.div>
