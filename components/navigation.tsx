        ...
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex min-h-16 items-center justify-between gap-3">
            ...
            <nav className="hidden items-center gap-1 md:flex" aria-label={locale === "pt" ? "Navegação principal" : "Main navigation"}>
              {[{ label: t.home, href: "#home" }, { label: t.services, href: "#services" }, { label: t.work, href: "#work" }, { label: t.about, href: "#about" }].map((link) => (
                <a key={link.href} href={link.href} onClick={(e) => { e.preventDefault(); handleNavClick(link.href); }} className="group relative rounded-md px-3 py-2 text-sm text-[#7a7a7a] transition-colors hover:bg-white/[0.04] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40">
                  {link.label}
            ...
            <div className="hidden items-center gap-2 md:flex">
              <button type="button" onClick={toggleLocale} className="inline-flex h-9 items-center gap-1.5 rounded-full border border-white/10 px-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#888] transition hover:border-white/20 hover:text-white" aria-label={`${copy[locale].language}: ${locale === "pt" ? "English" : "Português"}`}>
                <span className={locale === "pt" ? "text-white" : ""}>PT</span><span className="text-[#444]">/</span><span className={locale === "en" ? "text-white" : ""}>EN</span>
              </button>
              <a href="https://www.instagram.com/silentra.dev/" target="_blank" rel="noreferrer" className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-[#7a7a7a] transition hover:border-white/20 hover:bg-white/[0.05] hover:text-white" aria-label={t.instagram}><InstagramIcon /></a>
              <a href="#contact" onClick={(e) => { e.preventDefault(); handleNavClick("#contact"); }} className="group inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.07] px-4 py-2 text-sm font-medium text-white transition hover:border-white/25 hover:bg-white/[0.11] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50">
                {t.talk}<ArrowUpRight size={14} className="transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" aria-hidden="true" />
              </a>
            </div>

            <div className="flex items-center gap-2 md:hidden">
              <button type="button" onClick={toggleLocale} className="inline-flex h-10 items-center rounded-full border border-white/10 px-2.5 text-[10px] font-semibold tracking-[0.1em] text-white transition hover:border-white/20" aria-label={copy[locale].language}>{locale === "pt" ? "EN" : "PT"}</button>
              <button type="button" onClick={() => setMobileMenuOpen((open) => !open)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.03] text-white transition hover:border-white/20 hover:bg-white/[0.07] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50" aria-label={mobileMenuOpen ? (locale === "pt" ? "Fechar menu" : "Close menu") : (locale === "pt" ? "Abrir menu" : "Open menu")} aria-expanded={mobileMenuOpen} aria-controls="mobile-radial-menu">
                <AnimatePresence initial={false} mode="wait">
                  <motion.span key={mobileMenuOpen ? "close" : "open"} initial={{ opacity: 0, rotate: -90, scale: 0.7 }} animate={{ opacity: 1, rotate: 0, scale: 1 }} exit={{ opacity: 0, rotate: 90, scale: 0.7 }} transition={{ duration: 0.2 }} className="inline-flex">
