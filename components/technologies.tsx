...
export function Technologies() {
  ...
  const t = copy[locale].tech;
  return (
    <section
      className="relative overflow-hidden py-24"
      aria-label={locale === "pt" ? "Tecnologias" : "Technologies"}
    >
      <div
        aria-hidden="true"
        ...
        >
          {technologies.map((tech, i) => (
            <motion.span
              ...
              className="inline-flex items-center rounded-full border border-white/[0.08] bg-white/[0.03] px-4 py-2 text-xs font-medium text-[#b8b8b8] transition-all hover:border-white/[0.16] hover:bg-white/[0.06] hover:text-white"
            >
              {tech}
            </motion.span>
          ))}
